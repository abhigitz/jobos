...
class Job(Base, IDMixin, TimestampMixin):
    ...
    keywords_matched: Mapped[Optional[List[str]]] = mapped_column(StringArray())
    keywords_missing: Mapped[Optional[List[str]]] = mapped_column(StringArray())
    ai_analysis: Mapped[Optional[Dict]] = mapped_column(JSONBCompat())
    fit_reasoning: Mapped[Optional[str]] = mapped_column(Text)
    salary_range: Mapped[Optional[str]] = mapped_column(String(100))
    resume_suggestions: Mapped[Optional[list]] = mapped_column(JSONBCompat())
    ...
    notes: Mapped[Optional[list]] = mapped_column(JSONBCompat(), server_default='[]', default=list)
